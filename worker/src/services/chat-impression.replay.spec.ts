...
class InMemoryBackend {
  ...
  private nextIso(): string {
    const value = new Date(
      Date.UTC(2026, 3, 11, 12, 0, this.tick),
    ).toISOString();
    this.tick += 1;
    return value;
  }
  ...
  private matchesFilter(payload: Record<string, any>, filter?: any): boolean {
    ...
    return must.every(
      (condition: any) => payload?.[condition.key] === condition?.match?.value,
    );
  }

  private buildLeafPointsByLineIds(
    lineIds: string[],
  ...
  async post(url: string, body: any) {
    ...
    if (url.includes('/points/scroll')) {
      const points = Array.from(this.qdrantPoints.values())
        ...
        .sort(
          (left, right) =>
            new Date(
              right.payload.lineLastActivatedAt ||
                right.payload.updatedAt ||
                right.payload.createdAt,
            ).getTime() -
            new Date(
              left.payload.lineLastActivatedAt ||
                left.payload.updatedAt ||
                left.payload.createdAt,
            ).getTime(),
        )
    ...
    if (url.includes('/internal/memory/lines/keyword-search')) {
      ...
      const lines = Array.from(this.lines.values())
        ...
        .filter(
          (line) =>
            `${line.anchorLabel} ${line.impressionLabel} ${line.impressionAbstract}`.includes(
              query.slice(0, 4),
            ) ||
            `${line.anchorLabel} ${line.impressionLabel} ${line.impressionAbstract}`.includes(
              query,
    ...
    if (url.includes('/internal/memory/lines/by-ids')) {
      const lines = (body.lineIds || [])
        ...
        .filter((line: StoredLine | undefined): line is StoredLine =>
          Boolean(line),
        );
      return { data: lines };
    }
  ...
  async patch(url: string, body: any) {
    if (
      url.includes('/internal/memory/lines/') &&
      url.endsWith('/impression')
    ) {
...
function createService(
  ...
  mockedAxios.put.mockImplementation((url: string, body?: any) =>
    backend.put(url, body),
  );
  mockedAxios.post.mockImplementation((url: string, body?: any) =>
    backend.post(url, body),
  );
  mockedAxios.patch.mockImplementation((url: string, body?: any) =>
    backend.patch(url, body),
  );
...
describe('memory architecture replay', () => {
  ...
  it('updates same-day source points in place and writes revision logs', async () => {
    ...
    const dashscopeService = {
      ...
      generateRetrievalDrafts: jest.fn(
        async ({ messages }: { messages: ChatMessageInput[] }) =>
          buildDrafts(messages),
      ),
      generateNode2Points: jest
        ...
        .mockImplementationOnce(
          async ({ oldPoints }: { oldPoints: Array<{ id: string }> }) => [
            {
              op: 'revise',
      ...
      messages: createMessages([
        {
          id: 3,
          role: 'user',
          content: '对，你不能假装看过，只能基于资料',
          isNew: true,
        },
        {
          id: 4,
          role: 'assistant',
          content: '明白，我会按这个边界来讲',
          isNew: true,
        },
      ]),
    });
      ...
      attachPointToExistingLine: jest
        ...
        .mockImplementationOnce(
          async ({
            ...
            candidateLines: Array<{ id: string }>;
          }) => ({
            targetLineId: candidateLines[0]?.id || null,
          }),
        ),
      planNewLines: jest.fn(
        async ({ pointTexts }: { pointTexts: string[] }) => ({
  ...
  it('creates multiple new lines for unrelated new points in the same batch', async () => {
    ...
    expect(
      Array.from(backend.lines.values())
        .map((line) => line.anchorLabel)
        .sort(),
    ).toEqual(['聊咖啡豆选择', '聊跑步训练']);
